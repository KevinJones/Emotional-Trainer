...
Filename: editChildProfile.php
Description: This file contains code for editing the child profile.
...
include("../caregiver/include/phpFunctions.php");
?>
...
<form id="newChild" name="newChild" class="appnitro" method="post" enctype="multipart/form-data" action="editChildProfileSubmit.php" onSubmit="return ValidateChildProfile()">
 ...
 <label class="description" for="username">* Username </label><input id="username" name= "username" class="element text" maxlength="255" value=
			"<?php 
			$db=connectDB();
			$string = "SELECT Username from Child WHERE CaregiverID = '{$_COOKIE["CaregiverID"]}';";
 ...
 <label class="description" for="realName">Given Name</label><input id="realName" name= "realName" class="element text" maxlength="255" value=
			"<?php 
			$db=connectDB();
			$string = "SELECT Name from Child WHERE CaregiverID = '{$_COOKIE["CaregiverID"]}';";
			$childSelectionResource = mysql_query($string);
 ...
 <li id="li_6" >
 		The current date of birth is : <?php  
			$db=connectDB();
			$string = "SELECT DOB from Child WHERE CaregiverID = '{$_COOKIE["CaregiverID"]}';";
			$childSelectionResource = mysql_query($string);
...
<select name="autismClassification" id="autismClassification">
      <option value="
			<?php 
			$db=connectDB();
			$string = "SELECT AutisticSpec from Child WHERE CaregiverID = '{$_COOKIE["CaregiverID"]}';";
			$childSelectionResource = mysql_query($string);
